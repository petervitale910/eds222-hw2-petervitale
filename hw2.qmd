...
ggplot(trout_sample, aes(x = lake_trout, y = cutthroat))+
  ...
  labs(x = 'Lake Trout',
       ...
       subtitle = 'Plot of 300 random points (originally 100,000 points)')+
...
trout_lm <- lm(cutthroat ~ lake_trout, trout_sp);summary(trout_lm)
...
n_30 <- map_dbl(1:1000, ~ p_dist(30))
n_30_fraction <- sum(n_30 < .05) / length(n_30)

n_50 <-map_dbl(1:1000, ~ p_dist(50))
n_50_fraction <- sum(n_50 < .05) / length(n_50)

n_100 <-map_dbl(1:1000, ~ p_dist(100))
n_100_fraction <- sum(n_100 < .05) / length(n_100)
